...
CREATE TABLE Customer (customerId BIGINT PRIMARY KEY NOT NULL, username STRING, password STRING, role STRING);

-- Table: Manufacturer
DROP TABLE IF EXISTS Manufacturer;
CREATE TABLE Manufacturer (manufacturerId BIGINT PRIMARY KEY NOT NULL, name STRING, country STRING, businessIdentityCode STRING);
...
CREATE TABLE Product (productId BIGINT PRIMARY KEY NOT NULL, name STRING, productType BIGINT REFERENCES ProductType (productTypeId) ON UPDATE CASCADE NOT NULL, color STRING, size STRING, price DECIMAL (5, 2) NOT NULL, inStock INTEGER, manufacturer BIGINT REFERENCES Manufacturer (manufacturerId) ON UPDATE CASCADE NOT NULL);
...
CREATE TABLE ProductType (productTypeId BIGINT PRIMARY KEY, productTypeValue STRING);
